...
namespace LoginRegDemo.Controllers {
    public class HomeController : Controller {
        ...
        public IActionResult SingleEvent (int id) {
            ...
            List<Like> likes = _context.events.Include(ig => ig.user).Include(pa => pa.Participants).ThenInclude(u => u.user).SingleOrDefault(uq => uq.EventId == id).ToList();
        ...
        public IActionResult Unjoin (int id)
        {
            ...
            Participant Participant = _context.participants.SingleOrDefault(p => p.UserId == users.UserId && p.EventId == id);
